...
module Data.Analytics.Approximate.HyperLogLog where
...
castHLL :: forall n m. (Reifies n Int, Reifies m Int) => HLL n -> HLL m
castHLL old = new { buckets = newV } where
 (new :: HLL m) = initHLL
 ...
 targetIndexes i
   ...
   | otherwise = error "castHLL: Can't upcast HLL to a higher accuracy yet."
...
instance Reifies n Int => Monoid (HLL n) where
  mempty = initHLL :: HLL n
  mappend a b = HLL (V.zipWith max (buckets a) (buckets b)) (meta a)

-- | Initialize an HLL of given size 'n'
initHLL :: forall n. Reifies n Int => HLL n
initHLL = HLL (V.replicate (numBuckets cs) 0) cs
  where (cs :: Constants n) = mkConst

-- | update the counter with a new observation
updateHLL :: Hashable a => HLL n -> a -> HLL n
updateHLL (HLL !v cs) w = HLL v' cs where
  (!bk, !rnk) = bucketAndRank cs $ fromIntegral $ hash w -- asWord32 $ hash32 w
  modFn x = do
    old <- MV.read x bk
...
cardinality (HLL bs Constants{..}) = round res where
...
mkConst :: forall n. Reifies n Int => Constants n
mkConst = Constants
  { bits = b'
  , numBuckets = m'
  , smallRange = 5/2 * m''
...
bucketAndRank Constants{..} x = (bk, rank) where
